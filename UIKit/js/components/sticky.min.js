...
!function (t) {
	...
	function i() {
		...
		if (i.length && !(e.scrollTop() < 0))for (var o, a, r, p, h = e.scrollTop(), c = s.height(), l = e.height(), d = c - l, u = h > d ? d - h : 0, m = 0; m < i.length; m++)if (p = i[m], p.element.is(":visible") && !p.animate) {
			if (p.check()) {
				if (p.top < 0 ? o = 0 : (r = p.element.outerHeight(), o = c - r - p.top - p.options.bottom - h - u, o = 0 > o ? o + p.top : p.top), p.boundary && p.boundary.length) {
					var f = p.boundary.offset().top;
					a = p.boundtoparent ? c - (f + p.boundary.outerHeight()) + parseInt(p.boundary.css("padding-bottom")) : c - f - parseInt(p.boundary.css("margin-top")), o = h + r > c - a - (p.top < 0 ? 0 : p.top) ? c - a - (h + r) : o
				}
				if (p.currentTop != o) {
					if (p.element.css({
							position: "fixed",
						...
						g.length && setTimeout(function (t, i) {
							return function () {
								...
								n.top < s && e.top < a && (h = e.top - o - i.options.target, window.scrollTo(0, h))
							}
						}(g, p), 0)
					}
					p.element.addClass(p.options.clsactive).removeClass(p.options.clsinactive), p.element.trigger("active.uk.sticky"), p.element.css("margin", ""), p.options.animation && p.init && !t.Utils.isInView(p.wrapper) && p.element.addClass(p.options.animation), p.currentTop = o
				}
		...
		defaults: {
			...
			this.wrapper = this.element.css("margin", 0).wrap(a).parent(), this.computeWrapper(), r && (r === !0 || "!" === r[0] ? (r = r === !0 ? this.wrapper.parent() : this.wrapper.closest(r.substr(1)), i = !0) : "string" == typeof r && (r = t.$(r))), this.sticky = {
				...
				check: function () {
					...
					var i = e.scrollTop(), n = s.height(), a = n - window.innerHeight, r = i > a ? a - i : 0, p = this.wrapper.offset().top, h = p - this.top - r, c = i >= h;
